...
def extract_features(samples, transform):
    ...
    for frames, label in samples:
        ...
        #         batch_frames = transformed_frames[i:i + batch_size]
        #         batch_tensor = torch.stack(batch_frames).to(device)
        #         batch_features = resnet_feat(batch_tensor)
...
def splittingData(samples):
    ...
    np.save('./src/features/train_features.npy', train_features)
    np.save('./src/features/train_labels.npy', train_labels)
    np.save('./src/features/val_features.npy', val_features)
    np.save('./src/features/val_labels.npy', val_labels)
    np.save('./src/features/test_features.npy', test_features)
    np.save('./src/features/test_labels.npy', test_labels)
