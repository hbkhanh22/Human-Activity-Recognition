...
def loadFeatures():
    ...
    train_features = torch.from_numpy(np.load('./src/features/train_features.npy')).float()
    train_labels = torch.from_numpy(np.load('./src/features/train_labels.npy'))
    idx = np.random.permutation(len(train_features))
    train_features, train_labels = train_features[idx], train_labels[idx]

    val_features = torch.from_numpy(np.load('./src/features/val_features.npy')).float()
    val_labels = torch.from_numpy(np.load('./src/features/val_labels.npy'))

    test_features = torch.from_numpy(np.load('./src/features/test_features.npy')).float()
    test_labels = torch.from_numpy(np.load('./src/features/test_labels.npy'))

    return train_features, train_labels, val_features, val_labels, test_features, test_labels


def trainModel(train_features, train_labels, val_features, val_labels, num_epochs=100, num_frames=32, hidden_size=256, learning_rate=0.001):
    """
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig('./src/results/loss_plot.png')
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig('./src/results/accuracy_plot.png')
    plt.close()
